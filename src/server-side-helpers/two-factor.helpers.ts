...
export async function sendTwoFactorCode(email: string, code: string) {
    try {
        const content = `
            ...
            <p>Hello,</p>
            <p>You are attempting to sign in to your Diwa Date account. To complete the login process, please enter the following 6-digit verification code:</p>
 ...
 */
export async function generateAndSendTwoFactorCode(userId: number) {
    try {
        ...
        log(`2FA code generated and sent for user ${userId}`, 'info');
        ...
        log('Error generating and sending 2FA code', 'error');
        logError(error);
        return {
            success: false,
