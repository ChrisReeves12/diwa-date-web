...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userResult.success) {
      // User not found and this is a login attempt
      if (!isSignup) {
        return NextResponse.redirect(
          new URL(
            `/login?error=${encodeURIComponent(userResult.error || 'User not found')}`,
            getBaseUrl(request)
          )
        );
      }

      // For signup, if we get here without a userId, something went wrong
      return NextResponse.redirect(
        new URL(`/registration?error=${encodeURIComponent(userResult.error || 'Registration failed')}`, request.url)
      );
    }

    // If signup flow and no userId, redirect to registration with pre-filled data
    if (isSignup && !userResult.userId) {
