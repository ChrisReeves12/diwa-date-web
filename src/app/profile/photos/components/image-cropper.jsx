...
function ImageCropper({ imageFile, onCropChange, aspectRatio, className = '' }) {
    const canvasRef = (0, react_1.useRef)(null);
    const containerRef = (0, react_1.useRef)(null);
    const [imageLoaded, setImageLoaded] = (0, react_1.useState)(false);
    const [isDragging, setIsDragging] = (0, react_1.useState)(false);
    const [dragType, setDragType] = (0, react_1.useState)(null);
    ...
    const drawCropOverlay = (ctx, canvasWidth, canvasHeight) => {
        ...
        if (image) {
            ctx.drawImage(image, cropArea.x, cropArea.y, cropArea.width, cropArea.height, scaledCrop.x, scaledCrop.y, scaledCrop.width, scaledCrop.height);
        }
        // Draw crop border
    ...
    const getHitArea = (mouseX, mouseY) => {
        const scaledCrop = {
            x: cropArea.x * scale,
            y: cropArea.y * scale,
            width: cropArea.width * scale,
        ...
        if (mouseX >= scaledCrop.x - handleSize / 2 && mouseX <= scaledCrop.x + handleSize / 2 &&
            mouseY >= scaledCrop.y - handleSize / 2 && mouseY <= scaledCrop.y + handleSize / 2) {
            return 'resize-tl';
        }
        if (mouseX >= scaledCrop.x + scaledCrop.width - handleSize / 2 && mouseX <= scaledCrop.x + scaledCrop.width + handleSize / 2 &&
            mouseY >= scaledCrop.y - handleSize / 2 && mouseY <= scaledCrop.y + handleSize / 2) {
            return 'resize-tr';
        }
        if (mouseX >= scaledCrop.x - handleSize / 2 && mouseX <= scaledCrop.x + handleSize / 2 &&
        ...
        if (mouseX >= scaledCrop.x && mouseX <= scaledCrop.x + scaledCrop.width &&
    ...
    const handleMouseMove = (0, react_1.useCallback)((e) => {
        ...
        if (isDragging && image) {
            ...
            const newCrop = Object.assign({}, cropArea);
            if (dragType === 'move') {
                // Move the crop area
            ...
            else if (hitArea === null || hitArea === void 0 ? void 0 : hitArea.startsWith('resize')) {
    ...
    return (<div ref={containerRef} className={`image-cropper ${className}`} style={{ width: '100%', height: '400px', position: 'relative' }}>
      <canvas ref={canvasRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} style={{
...
//# sourceMappingURL=image-cropper.jsx.map